...
namespace DynoSharp
{
    ...
    internal static class DynoBrain
    {
        ...
        private static Mat CactusTemplate = null!;
        private static Mat OlympicTorchTemplate = null!;

        private static readonly double[] ThresholdValues = 
        {
            0.9,
            0.7
        };
        ...
        {
            // Mat Type: CV_8UC4 (4 colors: BGRA, unsigned)
            Mat tempCactus = new Mat(@"ImageReference/cactus.png", ImreadModes.Unchanged);
            Mat tempOlympicTorch = new Mat(@"ImageReference/olympic_torch.png", ImreadModes.Unchanged);

            CactusTemplate = tempCactus.CvtColor(ColorConversionCodes.BGRA2GRAY);
            OlympicTorchTemplate = tempOlympicTorch.CvtColor(ColorConversionCodes.BGRA2GRAY);
            
            tempCactus.Dispose();
            tempOlympicTorch.Dispose();
        }

        private static void Shutdown()
        {
            CactusTemplate.Dispose();
            OlympicTorchTemplate.Dispose();
        }

        private static void OpenCvDetection()
        {
            int rowCactus = ScreenFrame.Rows - CactusTemplate.Rows + 1;
            int colCactus = ScreenFrame.Cols - CactusTemplate.Cols + 1;
            int rowOlympicTorch = ScreenFrame.Rows - OlympicTorchTemplate.Rows + 1;
            int colOlympicTorch = ScreenFrame.Cols - OlympicTorchTemplate.Cols + 1;
            Mat resultCactus = new Mat(rowCactus, colCactus, MatType.CV_32FC1);
            Mat resultOlympicTorch = new Mat(rowOlympicTorch, colOlympicTorch, MatType.CV_32FC1);

            Mat screenFrameTemplate = ScreenFrame.CvtColor(ColorConversionCodes.BGRA2GRAY);

            (Mat, Mat)[] matchingTemplates = 
            {
                (CactusTemplate, resultCactus),
                (OlympicTorchTemplate, resultOlympicTorch)
            };
            
            Parallel.For(0, matchingTemplates.Length, i =>
            {
                OpenCV.MatchTemplate(
                    // ReSharper disable once AccessToDisposedClosure
                    screenFrameTemplate,
                    matchingTemplates[i].Item1,
                ...
                while (true) {
                    ...
                    if (maxValue < ThresholdValues[i]) break;

                    if (maxLocation.X < ScreenFrame.Width * JUMP_RATIO) {
                ...
                matchingTemplates[i].Item2.Dispose();
            });
